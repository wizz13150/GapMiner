...
void ChineseSet::init() {
  ...
  SieveUtils utils(n_primes);
  utils.primorial(mpz_primorial, n_primes);

  this->max_merit = size / (SieveUtils::mpz_log(mpz_primorial) + log(2) * (256 + 20));
  ...
  /* generate the pre sieve number range */
  utils.run_sieve(mpz_offset, size);
  this->sieve = utils.get_sieve();

  /* check the candidates */
  sieve_t n = 0;
  for (sieve_t i = 1; i < size; i += 2)
    if (is_prime(sieve, i))
      n++;
